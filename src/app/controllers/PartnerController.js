...
class PartnerController {
    ...
    async store(req, res, next) {
        try {
            ...
            if (!name) {
                return res.status(400).json({ message: "Tên đối tác không được để trống" });
            }
            ...
            // Thêm đối tác
            const partner = new Partner({ name, logo: logoPath, link, project: projectId });
            await partner.save();
    ...
    edit(req, res, next) {
                            Partner.findById(req.params.id).lean()
    ...
    update(req, res, next) {
                    ...
                    trash(req, res, next) {
                        Partner.findDeleted({})
                            .then(partner => {
                                res.render('admin/partner/trash', { partner: mutipleMongooseToObject(partner) })
                            })
